...
std::unique_ptr<NoviceSystem::PipelineSet> NoviceSystem::CreateGraphicsPipeline(
  D3D12_PRIMITIVE_TOPOLOGY_TYPE topologyType, BlendMode blendMode) {
	...
	// 頂点シェーダの読み込みとコンパイル
	std::wstring vsFile = kResourceRoot + L"shaders/ShapeVS.hlsl";
	...
	// ピクセルシェーダの読み込みとコンパイル
	std::wstring psFile = kResourceRoot + L"shaders/ShapePS.hlsl";
	result = D3DCompileFromFile(
...
void NoviceSystem::DrawBox(int x, int y, int w, int h, float angle, unsigned int color) {
	...
	// 頂点データ
	vertices = {
	  {{left, bottom, 0.0f},  {1, 1, 1, 1}}, // 左下
	  {{left, top, 0.0f},     {1, 1, 1, 1}}, // 左上
	  {{right, top, 0.0f},    {1, 1, 1, 1}}, // 右上
	  {{right, bottom, 0.0f}, {1, 1, 1, 1}}, // 右下
	};
	indices = {0, 1, 2, 2, 3, 0};
...
void NoviceSystem::DrawTriangle(
  int x1, int y1, int x2, int y2, int x3, int y3, unsigned int color) {
	...
	// 頂点データ
	vertices = {
	  {{static_cast<float>(x1), static_cast<float>(y1), 0.0f}, {1, 1, 1, 1}},
	  {{static_cast<float>(x2), static_cast<float>(y2), 0.0f}, {1, 1, 1, 1}},
	  {{static_cast<float>(x3), static_cast<float>(y3), 0.0f}, {1, 1, 1, 1}},
	};
	...
	// 頂点バッファへのデータ転送
	std::copy(vertices.begin(), vertices.end(), &triangle_->vertMap[indexVertex]);
...
void NoviceSystem::DrawQuad(
  ...
  int srcH, int textureHandle, unsigned int color) {
	...
	// 頂点データ
	std::vector<Sprite::VertexPosUv> vertices = {
	  {
...
void Novice::Initialize(const char* title, int width, int height) {
	...
	sAudio->Initialize(kResourceRootChar);

	// テクスチャマネージャの初期化
	TextureManager::GetInstance()->Initialize(sDxCommon->GetDevice(), kResourceRootChar);
	TextureManager::Load("white1x1.png");

	// スプライト静的初期化
	Sprite::StaticInitialize(sDxCommon->GetDevice(), width, height, kResourceRoot);
...
void Novice::DrawEllipse(
  int x, int y, int radiusX, int radiusY, float angle, unsigned int color, FillMode fillMode) {
	...
	// 分割点。分割数より1つ多くしておいて1周回るようにする
	std::array<int, kNumDivision + 1> pointsX;
	std::array<int, kNumDivision + 1> pointsY;
	...
	for (size_t index = 0; index < kNumDivision; ++index) {
		...
		pointsX[index] = int(baseX * angleCos - baseY * angleSin + x);
		pointsY[index] = int(baseY * angleCos + baseX * angleSin + y);
	}
	// 最後と最初は一緒
	...
	if (fillMode == kFillModeSolid) {
		for (size_t index = 0; index < kNumDivision; ++index) {
			sNoviceSystem->DrawTriangle(
			  x, y, pointsX[index], pointsY[index], pointsX[index + 1], pointsY[index + 1], color);
		}
	} else {
		for (size_t index = 0; index < kNumDivision; ++index) {
			sNoviceSystem->DrawLine(
			  pointsX[index], pointsY[index], pointsX[index + 1], pointsY[index + 1], color);
		}
	}
}
...
void Novice::SetIcon(const char* fileName) {
	...
	if (2 < strlen(fileName) && fileName[0] == '.' && fileName[1] == '/') {
		...
		iconPath = kResourceRootChar + fileName;
	}
